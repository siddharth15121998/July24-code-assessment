...
def loginCheck(request):
    try:
        ...
        pacode=request.POST.get("pcode")
        paphone=request.POST.get("pphone")
        getPatient=Patient.objects.filter(pcode=pacode,pphone=paphone)
        pserialize=PatientSerializer(getPatient,many=True)

        if(pserialize.data):
            for i in pserialize.data:
                getid=i["id"]
                getpname=i['pname']
                getpaddress=i['paddress']
                getpemail=i['pemail']


            request.session['uid']=getid
            request.session['uname']=getpname
            data={"pname":getpname,"paddress":getpaddress,"pemail":getpemail}
